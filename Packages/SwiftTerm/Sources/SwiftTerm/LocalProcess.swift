    ...
    public func startProcess(executable: String = "/bin/bash", args: [String] = [], environment: [String]? = nil, execName: String? = nil, currentDirectory: String? = nil)
     {
        ...
        // Use forkpty on macOS: it properly calls login_tty() which establishes
        // the controlling terminal via setsid() + ioctl(TIOCSCTTY). The Subprocess
        // path (posix_spawn) cannot reliably set the controlling terminal, causing
        // shells like zsh to fail with "setpgid: Inappropriate ioctl for device".
