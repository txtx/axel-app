...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    public override func scrollWheel(with event: NSEvent) {
        ...
        // When mouse reporting is enabled and we're in a mode that tracks button presses,
        // send scroll wheel events to the application (e.g., tmux with mouse mode)
        if allowMouseReporting && terminal.mouseMode.sendButtonPress() {
            let displayBuffer = terminal.displayBuffer
            let hit = calculateMouseHit(with: event)
            let screenRow = max(0, min(displayBuffer.rows - 1, hit.grid.row - displayBuffer.yDisp))
            ...
            let flags = event.modifierFlags
            let wheelButton = event.deltaY > 0 ? 4 : 5
            ...
            let scrollCount = max(1, Int(abs(event.deltaY)))
